"""诊断 issue1 基线：不改优先级，看 PC1/real1 的 UF-CURE 与 PKPOV 占用"""
import os, sys, copy
...
for ln in ["PC1","real1","PC2","real2"]:
    ...
    print(f"\n[{ln}] priority={next(l.priority for l in lots if l.lot_name==ln)}:")
    for e in ents:
        if "UF-" in e.step_name or "FC-REFLOW" in e.step_name:
