...
@app.route('/api/manual-adjust/upload', methods=['POST'])
def upload_manual_adjusts():
    """上传手动调整规则文件并覆盖当前内存规则。

    接受 .csv（tab/逗号分隔）或 .xlsx，列: lot_name, step_name, delay_to, mode。
    仅更新内存缓存（不写回 data/manual_adjust.csv），点击"保存到文件"才持久化。
    """
    global _manual_adjusts_cache
    try:
        file = request.files.get('file')
        ...
        if filename.lower().endswith('.csv'):
            ...
            return jsonify({"error": "仅支持 .csv 或 .xlsx"}), 400

        required = {'lot_name', 'delay_to'}
        if not required.issubset(set(df.columns)):
            return jsonify({"error": "缺少必需列，需要至少: lot_name, delay_to (可选 step_name, mode)"}), 400

        adjusts = []
        skipped = 0
        for _, row in df.iterrows():
            lot_name = _safe_str_pd(row, "lot_name").strip()
            delay_str = _safe_str_pd(row, "delay_to").strip()
            if not lot_name or not delay_str:
                skipped += 1
                continue
            try:
                delay_to = datetime.strptime(delay_str, "%Y/%m/%d %H:%M")
            except ValueError:
                try:
                    delay_to = datetime.strptime(delay_str, "%Y-%m-%d %H:%M")
                except ValueError:
                    skipped += 1
                    continue
            mode = _safe_str_pd(row, "mode").strip().lower()
            if mode not in ("delay", "pin"):
                mode = "delay"
            step_name = _safe_str_pd(row, "step_name").strip() or ""
            adjusts.append({
                "lot_name": lot_name,
                "step_name": step_name,
                "delay_to": delay_to.strftime("%Y/%m/%d %H:%M"),
                "mode": mode,
            })

        _manual_adjusts_cache = adjusts
        return jsonify({
            "success": True,
            "count": len(adjusts),
            "skipped": skipped,
