...
class OptimizerConfig:
    ...
    qtight_min_margin: float = 30.0
    """紧 Q-time 安全余量下限（分钟）。实际余量 = max(预算 × qtight_safety_margin%, 该下限)。
    百分比对短 Q 段不公平（如 120min 的 20% 仅 24min 缓冲），下限保证任何紧段至少预留该分钟数。
    推荐范围: 10-60
    变大: 短紧段缓冲更足、更不易超 Q，但起点整体更晚
    变小: 短紧段更紧凑，但缓冲不足时更易被击穿"""

    qtime_shortfall_gradient: float = 3.0
    """安全余量缺口罚分强度（统一余量计量的强度倍数）。
    余量项与加权完工时间统一计量（同单位分钟）：
      - margin ≥ 安全余量：微小奖励（≤20 分钟/链，渐近饱和）；
      - margin < 安全余量：罚分 = 强度 × 100万 × (1-rel)³ 分钟，越接近 0 罚分越大，
        余量=0 时 ≈ 一次超Q违规的罚分量级，引导搜索远离"余量濒危"的解。
    推荐范围: 1-20
    变大: 余量不足罚得更重（更接近超Q违规），搜索结果更偏向留足安全余量
    变小: 罚分更轻，主目标更偏向压缩完工时间，余量不足更常见"""
    ...
    batch_wait_window: int = 240
    """恒组批（together=true）等待凑批窗口（分钟）。到达同一"批次步骤"（如 CURE）的时间差
    在此窗口内的不同 Lot 合并进同一次同炉治程。窗口越大越倾向凑满一炉减少设备次数，
    但会把先到 Lot 的出批时间推迟；应不超过该步骤常见紧 Q-time 预算（如 DISPENSE→CURE=240），
    避免先到 Lot 因等待被推出 Q-time。
    推荐范围: 30-480
    变大: 更倾向凑满一炉、设备次数更少，但先到 Lot 出批更晚、更易被推出 Q-time
    变小: 先到先开炉，等待更短，但设备开炉次数更多、利用率更低"""
    ...
    def parameter_meta() -> list[dict]:
        ...
        return [
            ...
            {"group": "Q-time 链 / 紧凑性", "key": "qtight_min_margin", "type": "float",
             "label": "Q-time 安全余量下限（分钟）", "default": 30, "min": 10, "max": 60, "step": 5,
             "desc": "紧 Q-time 起点延迟预留的绝对下限，实际余量 = max(预算 × 安全余量%, 下限)。百分比对短 Q 段不公平，下限保证短紧段至少预留该分钟缓冲。",
             "up": "短紧段缓冲更足、更不易超 Q，但起点整体更晚",
             "down": "短紧段更紧凑，但缓冲不足时更易被击穿"},
            {"group": "Q-time 链 / 紧凑性", "key": "qtime_shortfall_gradient", "type": "float",
             "label": "安全余量缺口罚分强度", "default": 3, "min": 1, "max": 20, "step": 1,
             "desc": "统一余量计量：余量低于安全余量时罚分 = 强度×100万×(1-余量/安全)³ 分钟，越接近 0 越大（≈超Q违规量级）；达标区仅微小奖励（≤20分钟/链）。",
             "up": "余量不足罚得更重（更接近超Q违规），更倾向留足安全余量",
             "down": "罚分更轻，更倾向压缩完工时间，但余量可能不足"},
            ...
            {"group": "Q-time 链 / 紧凑性", "key": "batch_wait_window", "type": "int",
             "label": "恒组批等待凑批窗口（分钟）", "default": 240, "min": 30, "max": 480, "step": 10,
             "desc": "together=true 设备（如 CURE）等待凑批的窗口：时间差在此窗口内的 Lot 合并同炉。窗口越大越倾向凑满一炉，但先到 Lot 出批更晚。",
             "up": "更倾向凑满一炉、设备次数更少，但先到 Lot 出批更晚、更易被推出 Q-time",
             "down": "先到先开炉，等待更短，但设备开炉次数更多"},
