...
class OptimizerConfig:
    ...
    """安全余量缺口罚分梯度（低于安全余量时每缺 1 分钟的得分罚分倍数，缺口越深单分钟罚分越高）。
    罚分 = gradient × (shortfall + shortfall²/(2×safe))：余量刚低于安全余量时按 1× 梯度起步，
    越接近耗尽按倍率递增（到 0 余量时单分钟罚分 = 2× 梯度），引导搜索把余量抬回安全余量之上。
    推荐范围: 1-20
    变大: 余量不足罚分更重，搜索结果更偏向留足安全余量，但完工时间可能更晚
    ...
    def parameter_meta() -> list[dict]:
        ...
        return [
            ...
            {"group": "Q-time 链 / 紧凑性", "key": "qtime_shortfall_gradient", "type": "float",
             "label": "安全余量缺口罚分梯度", "default": 3, "min": 1, "max": 20, "step": 1,
             "desc": "余量低于安全余量时每缺 1 分钟的得分罚分倍数，缺口越深单分钟罚分越高（0 余量时达 2 倍梯度），驱动搜索把余量抬回安全余量之上。",
             "up": "余量不足罚分更重，更倾向留足安全余量，但完工时间可能更晚",
